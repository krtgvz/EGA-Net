...
from models.EGA import get_ega
from datasets.dataset1 import *
...
def parse_args():
    ...
    parser.add_argument("--run_name", type=str, default="EGA-Net", help="run name")
    ...
    ## dataset
    parser.add_argument("--paths_file", type=str, default="datasets_txt/Casiav23.txt",
                        help="path to the file with input paths")
    ...
    parser.add_argument("--lambda_redge", type=float, default=0.1, help="redge-scale loss weight (omega)")
    ## log
...
def predict_loss(args, data, model,
    ...
    in_imgs = in_imgs.to(device=torch.device('cuda'), dtype=torch.float32)

    in_masks = in_masks.to(device=torch.device('cuda'), dtype=torch.float32)
    in_edges = in_edges.to(device=torch.device('cuda'), dtype=torch.float32)
    in_labels = in_labels.to(device=torch.device('cuda'), non_blocking=True).float()

    # predict
    out_edges, out_masks, ret2 = model(in_imgs)
    out_edges = torch.sigmoid(out_edges)
    out_masks = torch.sigmoid(out_masks)
    out_masks = out_masks.to(device=torch.device('cuda'), dtype=torch.float32)
    ...
    # Redge loss
    zero_tensor = torch.zeros_like(ret2)
    loss_redge = criterion_MSE(ret2, zero_tensor.cuda())
    # Image-scale loss (with GMP)
    out_labels = gmp(out_masks).squeeze()
    in_labels = in_labels.view(out_labels.size())
    loss_clf = criterion_BCE(out_labels, in_labels)

    # Total loss
    alpha = args.lambda_seg
    beta = args.lambda_clf
    omega = args.lambda_redge

    weighted_loss_seg = alpha * loss_seg
    weighted_loss_clf = beta * loss_clf
    weighted_loss_redge = omega * loss_redge
    weighted_loss_edg = (1.0 - alpha - beta - omega) * loss_edg
    loss = weighted_loss_seg + weighted_loss_clf + weighted_loss_edg + weighted_loss_redge

    return loss, weighted_loss_seg, weighted_loss_clf, weighted_loss_edg, weighted_loss_redge, in_imgs, in_masks, in_edges, out_masks, out_edges, ret2


def write_loss_to_file(loss_list, filename):
    with open(filename, 'w') as file:
        for i, loss in enumerate(loss_list):
            file.write(f'Epoch {i + 1}: {loss}\n')
...
class GemPool2d(nn.Module):
    ...
    def forward(self, x):
        ...
        # 计算均值池化
        x = x.mean(dim=(-2, -1)).clamp(min=self.eps).pow(1.0 / self.p)

        return x


class CustomPoolingBlock(nn.Module):
    def __init__(self, a):
        super(CustomPoolingBlock, self).__init__()
        self.a = a
        self.in_channels = 1
        ...
        # 1x1卷积层
        self.conv2 = nn.Conv2d(self.out_channels, 1, kernel_size=1)

        # Sigmoid激活函数
        self.sigmoid = nn.Sigmoid()

        # Gem池化层
        self.gem_pool1 = GemPool2d()
        self.gem_pool2 = GemPool2d()

    def forward(self, x):
        # 分支1：卷积、激活、Batch Norm、卷积、激活
        out1 = self.conv1(x)
        out1 = self.relu(out1)
        out1 = self.batch_norm(out1)
        out1 = self.conv2(out1)
        out1 = self.sigmoid(out1)
        out1 = self.gem_pool1(out1)
        # 分支2：Gem池化
        out2 = self.gem_pool2(x)

        # 混合两个分支
        output = (1 - self.a) * out1 + self.a * out2

        return output


# 指定要保存的文件名
train_loss_filename = './loss/train_loss.txt'
val_loss_filename = './loss/val_loss.txt'
Pixel_scale_Loss_filename = './loss/pixel_scale_Loss.txt'
Edge_Loss_list_filename = './loss/edge_loss.txt'
Image_scale_Loss_list_filename = './loss/image_scale_loss.txt'
redge_scale_Loss_list_filename = './loss/redge_scale_loss.txt'
...
def train(args, global_rank, sync, get_module,
    ...
    gmp = CustomPoolingBlock(a=0.9).cuda()
    # self convGem layer
    Total_Loss_list = []
    Pixel_scale_Loss_list = []
    Edge_Loss_list = []
    Image_scale_Loss_list = []
    Val_Loss_list = []
    ...
    for epoch in range(args.cond_epoch, args.n_epochs):
        ...
        print('Starting Epoch {}'.format(epoch)+f' at {current_time}')
        ...
        for step, data in enumerate(dataloader):
            ...
            loss, weighted_loss_seg, weighted_loss_clf, weighted_loss_edg, weighted_loss_redge, in_imgs, in_masks, in_edges, out_masks, out_edges, ret2 = predict_loss(
                args, data, model, criterion_BCE, criterion_MSE, gmp)
            ...
            #             epoch_total_redge +=weighted_loss_redge.item()
            ...
            if step != 0 and step % args.log_interval == 0 and global_rank == 0:
                print(f"[Epoch {epoch}/{args.n_epochs - 1}] [Batch {step}/{len(dataloader)}] "
                      ...
                      f"[Image-redge Loss {weighted_loss_redge:.3e}]"
                      f"")
                ...
                writer.add_scalar("Loss/redge-scale", weighted_loss_redge, curr_steps)
            ...
            if step != 0 and step % args.checkpoint_interval == 0 and global_rank == 0:
                save_checkpoints(checkpoint_dir, args.id, epoch, step, 0,
                                 model)
        Total_Loss_list.append(loss.item())
        Pixel_scale_Loss_list.append(weighted_loss_seg.item())
        Edge_Loss_list.append(weighted_loss_edg.item())
        Image_scale_Loss_list.append(weighted_loss_clf.item())
        #  Validation
        # ------------------
        # 调用函数将损失值写入文件
        write_loss_to_file(Total_Loss_list, train_loss_filename)
        write_loss_to_file(Pixel_scale_Loss_list, Pixel_scale_Loss_filename)
        write_loss_to_file(Edge_Loss_list, Edge_Loss_list_filename)
        write_loss_to_file(Image_scale_Loss_list, Image_scale_Loss_list_filename)
        #         write_loss_to_file(redge_scale_Loss_list, redge_scale_Loss_list_filename)
        # ------------------
        #  Validation
        # ------------------
        if (args.val_paths_file and val_sampler and val_dataloader):
            ...
            for step, data in enumerate(val_dataloader):
                with torch.no_grad():
                    loss, _, _, _, _, _, _, _, _, _, _ = predict_loss(args, data, model, criterion_BCE, gmp)
